 ...
 *       2. Modified files must not regress (compiled on master -> must compile on PR)
 */
 ...
 */
function checkReactCompilerCompliance(
  source: string,
  filename: string,
): CompilationResult {
  let hasError = false;
...
function printErrors(filename: string, errors: CompilerError[]): void {
  if (IS_CI) {
    console.log(
      `::group::${filename} (${errors.length} error${errors.length === 1 ? '' : 's'})`,
    );
  }
...
function checkFiles(inputs: string[], verbose: boolean): boolean {
  ...
  if (files.length === 0) {
    logWarn(
      `No ${FILE_EXTENSIONS.join('/')} files found matching the provided paths.`,
    );
    return true;
  }
 ...
 * Rule 2: Modified files must not regress (compiled on master -> must compile on PR).
 */
async function checkChangedFiles(
  ...
  const reactFiles = changedFiles.filter(
    f =>
      FILE_EXTENSIONS.some(ext => f.filename.endsWith(ext)) &&
      f.status !== 'removed',
  );
  ...
  for (const {filename, status, previousFilename} of reactFiles) {
    ...
    if (status === 'added') {
      if (result.status === 'failed') {
        failures.push({
          file: filename,
          reason:
            'New file contains components/hooks that fail to compile with React Compiler',
    ...
    if (result.status === 'failed') {
      ...
      try {
        const mainSource = Git.show('origin/master', mainPath);
      ...
      if (mainStatus === 'compiled') {
        failures.push({
          file: filename,
          reason:
            'File compiled on master but fails to compile on this branch (regression)',
          errors: result.errors,
        });
        logError(`FAILED  ${filename} (regression: compiled on master)`);
        printErrors(filename, result.errors);
      } else if (verbose) {
        logWarn(
          `WARNING  ${filename} (fails to compile, but also failed on master)`,
        );
      }
    } else if (verbose) {
      const label = result.status === 'compiled' ? 'COMPILED' : 'SKIPPED ';
  ...
  if (failures.length > 0) {
    logError(
      `React Compiler compliance check failed with ${failures.length} error(s).`,
    );
    console.log();
    logInfo(
      'See contributingGuides/REACT_COMPILER.md for help fixing these errors.',
    );
    return false;
  }
...
async function main() {
    ...
    positionalArgs: [
      {
        ...
        parse: val => {
          if (!(CLI_COMMANDS as readonly string[]).includes(val)) {
            throw new Error(
              `Invalid command. Must be one of: ${CLI_COMMANDS.join(', ')}`,
            );
          }
          return val;
        },
      },
      {
        name: 'files',
        description:
          'File paths, directories, or glob patterns to check (only for "check" command)',
        variadic: true,
        default: [],
      },
    ],
  ...
  switch (command) {
    case 'check':
      if (files.length === 0) {
        logError(
          'No paths specified. Usage: npm run react-compiler-compliance-check check <files|dirs|globs...>',
        );
        process.exit(1);
      }
