...
type ParsedNamedArgs<NamedArgs extends CLIConfig['namedArgs']> = {
  [K in keyof NonNullable<NamedArgs>]: InferStringArgParsedValue<
    NonNullable<NamedArgs>[K]
  >;
};
...
class CLI<TConfig extends CLIConfig> {
   ...
   */
  public readonly positionalArgs: ParsedPositionalArgs<
    TConfig['positionalArgs']
  >;
  ...
  constructor(private readonly config: TConfig) {
    ...
    this.flags = {
      ...Object.fromEntries(
        Object.keys(config.flags ?? {}).map(key => [key, false]),
      ),
      yes: false,
      no: false,
      help: false,
    ...
    try {
      ...
      const parsedPositionalArgs: Partial<
        Writable<typeof this.positionalArgs>
      > = {};
        ...
        if (rawArg.startsWith('--')) {
          ...
          if (rawArgName in this.flags) {
            ...
            if (rawArgValue) {
              ...
              if (
                !argValueBeforeParse ||
                argValueBeforeParse.startsWith('--')
              ) {
            ...
            parsedNamedArgs[rawArgName as keyof typeof parsedNamedArgs] =
              CLI.parseStringArg(
                argValueBeforeParse,
                rawArgName,
                spec,
              ) as ValueOf<typeof parsedNamedArgs>;
          } else {
          ...
          if (spec.variadic) {
            ...
            ] = collected as ValueOf<typeof parsedPositionalArgs>;
            break;
          }
          parsedPositionalArgs[spec.name as keyof typeof parsedPositionalArgs] =
            CLI.parseStringArg(rawArg, spec.name, spec) as ValueOf<
              typeof parsedPositionalArgs
            >;
          positionalIndex++;
        }
      }
        ...
        if (providedNamedArgs.has(name) && spec.supersedes) {
          for (const supersededArg of spec.supersedes) {
            ...
            if (providedNamedArgs.has(supersededArg)) {
              console.warn(
                `⚠️  Warning: --${supersededArg} is superseded by --${name} and will be ignored.`,
              );
            }
          }
        }
      ...
      for (const [name, spec] of Object.entries(config.namedArgs ?? {})) {
        if (name in parsedNamedArgs) {
          if (supersededArgs.has(name)) {
            parsedNamedArgs[name as keyof typeof parsedNamedArgs] =
              undefined as ValueOf<typeof parsedNamedArgs>;
          }
          ...
          parsedNamedArgs[name as keyof typeof parsedNamedArgs] =
            spec.default as ValueOf<typeof parsedNamedArgs>;
      ...
      for (const spec of config.positionalArgs ?? []) {
        if (!(spec.name in parsedPositionalArgs)) {
          if (spec.default !== undefined) {
            parsedPositionalArgs[
              spec.name as keyof typeof parsedPositionalArgs
            ] = spec.default as ValueOf<typeof parsedPositionalArgs>;
          } else if (spec.variadic) {
            parsedPositionalArgs[
              spec.name as keyof typeof parsedPositionalArgs
            ] = [] as ValueOf<typeof parsedPositionalArgs>;
          } else {
            throw new Error(
      ...
      this.positionalArgs =
        parsedPositionalArgs as unknown as typeof this.positionalArgs;
    } catch (err) {
  ...
  private printHelp(): void {
    ...
    const namedArgUsage = Object.keys(namedArgs)
      .map(key => `[--${key} <value>]`)
      .join(' ');
    const flagUsage = [...Object.keys(flags), '--yes', '--no', '--help']
    ...
    // Built-in flags
    console.log(
      `  --${'yes'.padEnd(20)} Automatically answer "yes" to all confirmation prompts.`,
    );
    console.log(
      `  --${'no'.padEnd(20)} Automatically answer "no" to all confirmation prompts.`,
    );
      ...
      for (const [name, spec] of Object.entries(namedArgs)) {
        const defaultLabel =
          spec.default !== undefined
            ? ` (default: ${SafeString(spec.default)})`
            : '';
        const supersededLabel =
          spec.supersedes && spec.supersedes.length > 0
            ? ` (supersedes: ${spec.supersedes.join(', ')})`
            : '';
        console.log(
          `  --${name.padEnd(20)} ${spec.description}${defaultLabel}${supersededLabel}`,
        );
      }
      console.log('');
    }
      ...
      for (const arg of positionalArgs) {
        const defaultLabel =
          arg.default !== undefined
            ? ` (default: ${SafeString(arg.default)})`
            : '';
        console.log(
          `  ${arg.name.padEnd(22)} ${arg.description}${defaultLabel}`,
        );
      }
      console.log('');
    }
  ...
  private static parseStringArg<T extends StringArg>(
    rawString: string,
    paramName: string,
    spec: T,
  ): InferStringArgParsedValue<T> {
