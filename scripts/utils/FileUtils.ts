...
const ERROR_MESSAGES = {
  ...
  INDEX_OUT_OF_BOUNDS: (sourceLength: number, index: number) =>
    `Index ${index} is out of bounds for source length ${sourceLength}`,
} as const;
...
const FileUtils = {
  ...
  getLineAndColumnFromIndex: (
    ...
    const column =
      lastLineBreakIndex === -1 ? index + 1 : index - lastLineBreakIndex;
    return {line, column};
  },
