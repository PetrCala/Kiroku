...
Status: **direction locked, scaffolding plan pending implementation**
...
| 3 | Statistical sophistication | **EWMA (λ ≈ 0.3) + percentile band-of-normal visible. Mann–Kendall used *invisibly* to gate copy. Change-point detection deferred.** Test result never displayed as a number. |
...
CONST.DRINK_DEFAULTS = {
  ...
  cocktail:    { ml: 250, abv: 0.10 },
  strong_shot: { ml: 40,  abv: 0.40 },
  weak_shot:   { ml: 40,  abv: 0.20 },
  other:       { ml: 200, abv: 0.10 },
};
```
...
| `src/libs/Statistics/types.ts` | **Lives, extended.** `DrinkEvent` added; `DayRollup` becomes a *derived* type from `aggregate(events, byDay, sumUnits)`. |
...
Each tab ships with at least one **hero chart** so no tab feels empty on day one. Bullet items are v1; items in *italics* are v1.x adds.
...
- *Mann–Kendall caption gating*: "your weekly units are trending down" (when significant), else neutral copy
...
- *ECDF of units-per-session* — "70% of your sessions are ≤ 3 units"
...
- Type-switching transitions (*Sankey-lite, defer if it bloats v1*)
