...
const [sessions] = useOnyx(ONYXKEYS.COLLECTION.DRINKING_SESSION, {
    selector: (sessions) =>
        Object.fromEntries(
...
const [sessionAttributes] = useOnyx(ONYXKEYS.DERIVED.SESSION_ATTRIBUTES, {
    selector: (data) => data?.sessions,
});

// BAD: Selector filters/maps a collection into an array — deepEqual on every item
const [archivedSessionIdsArray] = useOnyx(ONYXKEYS.COLLECTION.DRINKING_SESSION, {
...
const [archivedSessionIds] = useOnyx(ONYXKEYS.COLLECTION.DRINKING_SESSION, {
...
const [sessionSummaries] = useOnyx(ONYXKEYS.COLLECTION.DRINKING_SESSION, {
    selector: (sessions) =>
        Object.values(sessions ?? {}).filter((s) => s?.total === 0),
});
const hasEmptySessions = sessionSummaries.length > 0;
```
...
const [sessions] = useOnyx(ONYXKEYS.COLLECTION.DRINKING_SESSION);
const mappedSessions = Object.fromEntries(
    Object.entries(sessions ?? {}).map(([key, session]) => [
        key,
        {id: session?.id, title: session?.title, type: session?.type},
    ]),
...
const [sessionAttributes] = useOnyx(ONYXKEYS.DERIVED.SESSION_ATTRIBUTES);
const sessionList = sessionAttributes?.sessions;

// GOOD: No selector — filter and compute Set inline
const [drinkingSessions] = useOnyx(ONYXKEYS.COLLECTION.DRINKING_SESSION);
const archivedSessionIds = new Set(
    Object.entries(drinkingSessions ?? {})
        .filter(([, value]) => value?.isArchived)
        .map(([key]) => key),
);

// GOOD: Selector computes the final boolean directly
const [hasEmptySessions] = useOnyx(ONYXKEYS.COLLECTION.DRINKING_SESSION, {
    selector: (sessions) =>
        Object.values(sessions ?? {}).some((s) => s?.total === 0),
});
```
