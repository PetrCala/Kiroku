...
- **Reason**: During auth initialization the SDK reloads the persisted user over the network (`reloadAndSetCurrentUserOrClear` -> `_reloadWithoutSaving`) and only emits the first `onAuthStateChanged` once that settles. The SDK's own `NetworkTimeout` bounds it at 30-60s, so on a network that is up but black-holed (VPN tunnel with no internet behind it, captive portal, dead wifi) a cold start hangs the entire boot sequence (splash gates, InitialScreen auto-login spinner, Home redirect) for that long. The patch bounds the reload with a 3s timeout that rejects with `code: 'auth/network-request-failed'`, which takes the SDK's existing "network error: keep the persisted user" branch; the timer is cleared when the reload settles first and a late reload failure is consumed by the wrapper, so no boot leaves a stray unhandled rejection. A genuinely invalid token is still caught: the first real kiroku-api call returns 401 and signs the user out. Applied to both bundles Kiroku loads: `dist/rn` (native via Metro's `react-native` main field) and `dist/esm2017` (web via webpack's `browser` field).
