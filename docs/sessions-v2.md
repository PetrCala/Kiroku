...
**W4 (live on the lock screen, #1667)**

- The Android notification's `timeoutAfter` is an absolute deadline derived from the session's last activity, not the plain auto-close hours (§8). The server's sweep measures staleness from `max(last drink, start_time)`, so a fixed duration from the start would expire the notification during a long night that is still being logged. Each drink reposts the notification and pushes the deadline out, exactly as it pushes the sweep out.
- When the environment has no auto-close configured (no `config/auto_close_default_hours` and no per-user `auto_close_sessions_after_hours`, or an explicit `"never"`), no `timeoutAfter` is set at all. Nothing will close the session, so nothing should cap the notification; §8 assumes auto-close is always on.
