...
**W2 (capture rework and solo sessions onto ops, #1665)**

- **`set_times` is an op the §5.1 table does not list.** W2 has to express editing a session's start and end as an op, and the table has none: `start` and `end` are lifecycle ops, and folding a setter into them would blur what they mean. The session's **timezone** rides on the same payload, because it answers the same question (what clock those times read in) and would otherwise need a second new op for the timezone screen. It is the only op that can move a `start_time`, so it is also the only one that touches the `earliest_session_at` floor: a start that lowers the floor does so in the same batch, one that raises it recomputes from the full set, and a timezone-only change leaves the floor alone.
- **The owner's update-log entry is appended by the publisher, not folded into the op's multi-path update.** §5.2 step 3 puts the log in the same update as the data. The log entry needs the sequence number from the existing `UpdatePublisher`'s counter transaction, which cannot be known before the batch is built, and reimplementing sequencing inline would give session ops a second update-log writer next to every other route's. The data write is still one atomic multi-path update; only the log append is the separate write it already is everywhere else.
- **Ops apply only to a `schema_version: 2` session.** A legacy session's drinks live in `drinks` buckets, which have no entry ids for an op to name and which a schema 2 session must not carry alongside `entries` (kiroku-api#151). The server refuses an op against one, and the client keeps such a session on the whole-session path until the §11 backfill converts it. §5.6 reads as though every solo session moves onto ops at once; in practice the two move together, session by session, as the backfill runs.
- **`POST /v1/sessions/ops` refuses a shared session with a permission error until W6a.** The §5.1 table lists most ops as applying to shared sessions too, but nothing writes the canonical `shared_sessions` document or fans out projections yet, so applying a solo delta to a projection would silently diverge it. The refusal reads `shared: true` off the record the op would write (§4.5), which costs no extra read.
- **Permission failures answer HTTP 460 with `jsonCode: 460`**, upstream's code carried all the way up to the status line rather than a 200 with a 460 body. The app's queue decides retry-vs-drop from the HTTP status, so putting the code there is what makes a permission failure roll back cleanly and a network failure retry, which is what §5.3 asks for.
- **A live session's save diffs its meta only, never its entries.** Every entry change was already sent as its own op while the session ran, so diffing entries again at save time against a cached snapshot that may lag the queue would resend ops that already landed. An edit session, whose mutations stay local until it is saved, is diffed in full.
- **The server does not maintain `drinksTimeParts` on the op path.** It is a client-computed `Intl` cache the whole-session finalize used to precompute; the Statistics read path recomputes any timestamp missing from it and its backfill regenerates the map, so an op-written session is correct and pays the cold-path cost once instead of having every op carry derived data.

**W3 (session identity, #1666)**

- A session photo can be removed by its **owner only**, not yet by "the uploader or the admin" (§9). The two rules are the same thing for a solo session, whose owner is its only uploader, and the distinction only becomes real with shared sessions. `POST /v1/images/session-photo/delete` gets ownership structurally rather than by a check: it reads the record under the caller's own uid, so another user's photo is simply a 404. The admin case lands with W6.
- A photo's `w` and `h` (§4.2) are **reported by the client**, bounded rather than trusted (integers, 1 to 20000). The bytes go straight from the client to the bucket through a presigned PUT, so the server never sees them and cannot measure them; the picker reports what it actually produced. The size cap already bounds what can be stored, so the risk of a wrong value is a layout bug, not a storage one.
- A photo's id is the **uuid of its stored object** (the object name without its extension), not a fresh push id (§4.2 leaves `PhotoId` unspecified). Finalize is then idempotent: a client that retries after a dropped response rewrites one record instead of adding a second photo for one object. The same reasoning as W1's deterministic legacy entry ids.
- Session photos are read through **short-lived signed urls** (`GET /v1/images/session-photos`, 15 minutes), not the public-url path avatars use. An avatar is public-read because it is shown to anyone who can see the profile; a session photo is only as visible as its session, which a permanent public url cannot express. The urls are gated on both privacy axes, the per-user gate and the session's own `visibility`, and are held in client state rather than persisted, so a viewer who loses access is re-gated on their next read.
- Naming a legacy session, or making it private, writes `name` / `visibility` onto it **without promoting it** to schema 2; promotion stays the backfill's job (§11). The server therefore validates those meta fields for legacy payloads too, not only for `schema_version: 2` ones.
