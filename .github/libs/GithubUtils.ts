...
class GithubUtils {
  ...
  static getStagingDeployCash(): Promise<StagingDeployCashData> {
    return this.octokit.issues
      ...
      .then(({data}: ListForRepoResult) => {
        if (!data.length) {
          throw new Error(
            `Unable to find ${CONST.LABELS.STAGING_DEPLOY} issue.`,
          );
        }

        if (data.length > 1) {
          throw new Error(
            `Found more than one ${CONST.LABELS.STAGING_DEPLOY} issue.`,
          );
        }
   ...
   */
  static getStagingDeployCashData(
    ...
    try {
      ...
      throw new Error(
        `Unable to find ${CONST.LABELS.STAGING_DEPLOY} issue with correct data.`,
      );
    }
  }
  ...
  static getStagingDeployCashDeployBlockers(
    ...
    let deployBlockerSection: RegExpMatchArray | string | null =
    ...
    const deployBlockers = [
      ...deployBlockerSection.matchAll(
        new RegExp(
          `- \\[([ x])]\\s(${CONST.ISSUE_OR_PULL_REQUEST_REGEX.source})`,
          'g',
        ),
      ),
    ].map(match => ({
   ...
   */
  static fetchAllPullRequests(
    pullRequestNumbers: number[],
  ): Promise<OctokitPR[] | void> {
    ...
    return this.paginate(
      ...
      .then(prList =>
        prList.filter(pr => pullRequestNumbers.includes(pr.number)),
      )
      .catch(err => console.error('Failed to get PR list', err));
  }

  static getPullRequestMergerLogin(
   ...
   */
  static getArtifactByName(
    artifactName: string,
  ): Promise<OctokitArtifact | undefined> {
    return this.octokit.actions
