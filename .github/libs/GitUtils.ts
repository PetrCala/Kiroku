 ...
 */
function getPreviousExistingTag(tag: string, level: SemverLevel): string {
 ...
 */
function getCommitHistoryAsJSON(
  fromTag: string,
  toTag: string,
): Promise<CommitType[]> {
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const sanitizedOutput = stdout.replace(
      /(?<="subject": ").*?(?="})/g,
      subject => sanitizeStringForJSONParse(subject),
    );
...
function getValidMergedPRs(commits: CommitType[]): number[] {
  ...
  commits.forEach(commit => {
    ...
    if (author === CONST.KIROKU_ADMIN) {
      return;
    }

    const match = commit.subject.match(
      /Merge pull request #(\d+) from (?!Expensify\/.*-cherry-pick-staging)/,
    );
...
async function getPullRequestsMergedBetween(fromTag: string, toTag: string) {
  ...
  console.log(
    `List of pull requests merged between ${fromTag} and ${toTag}`,
    pullRequestNumbers,
  );
