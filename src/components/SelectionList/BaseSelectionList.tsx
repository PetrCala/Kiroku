...
function BaseSelectionList<TItem extends ListItem>(
  ...
  const [maxToRenderPerBatch, setMaxToRenderPerBatch] = useState(
    shouldUseDynamicMaxToRenderPerBatch
      ? 0
      : CONST.MAX_TO_RENDER_PER_BATCH.DEFAULT,
  );
  const [isInitialSectionListRender, setIsInitialSectionListRender] =
    useState(true);
    ...
    sections.forEach((section, sectionIndex) => {
      ...
      section.data?.forEach((item, optionIndex) => {
        ...
        const isItemDisabled =
          !!section.isDisabled || (item.isDisabled && !item.isSelected);
        ...
        if (
          item.isSelected &&
          !selectedOptions.find(option => option.keyForList === item.keyForList)
        ) {
    ...
    return {
      ...
      allSelected:
        ...
        selectedOptions.length ===
          allOptions.length - disabledOptionsIndexes.length,
    };
  }, [
    canSelectMultiple,
    sections,
  ...
  const [slicedSections, ShowMoreButtonInstance] = useMemo(() => {
    let remainingOptionsLimit =
      CONST.MAX_SELECTION_LIST_PAGE_LENGTH * currentPage;
    const processedSections = getSectionsWithIndexOffset(
      sections.map(section => {
        const data =
          !isEmpty(section.data) && remainingOptionsLimit > 0
            ? section.data.slice(0, remainingOptionsLimit)
            : [];
  ...
  const disableEnterShortcut =
    activeElementRole &&
    [CONST.ROLE.BUTTON, CONST.ROLE.CHECKBOX].includes(
      activeElementRole as ButtonOrCheckBoxRoles,
    );
  ...
  const scrollToIndex = useCallback(
    (index: number, animated = true) => {
      ...
      listRef.current.scrollToLocation({
        ...
        itemIndex,
        animated,
        viewOffset: variables.contentHeaderHeight,
      });
    },
  ...
  const [disabledArrowKeyIndexes, setDisabledArrowKeyIndexes] = useState(
    flattenedSections.disabledArrowKeyOptionsIndexes,
  );
  useEffect(() => {
    if (
      arraysEqual(
        disabledArrowKeyIndexes,
        flattenedSections.disabledArrowKeyOptionsIndexes,
      )
    ) {
      return;
  ...
  const [focusedIndex, setFocusedIndex] = useArrowKeyFocusManager({
    initialFocusedIndex: flattenedSections.allOptions.findIndex(
      option => option.keyForList === initiallyFocusedOptionKey,
    ),
    maxIndex: Math.min(
      flattenedSections.allOptions.length - 1,
      CONST.MAX_SELECTION_LIST_PAGE_LENGTH * currentPage - 1,
    ),
  ...
  const selectRow = (item: TItem, indexToFocus?: number) => {
    ...
    if (canSelectMultiple) {
      if (sections.length > 1) {
        ...
        const selectedOptionsCount = item.isSelected
          ? flattenedSections.selectedOptions.length - 1
          : flattenedSections.selectedOptions.length + 1;
  ...
  const selectFocusedOption = () => {
    ...
    if (
      !focusedOption ||
      (focusedOption.isDisabled && !focusedOption.isSelected)
    ) {
      return;
    }
   ...
   */
  const getItemLayout = (
    data: Array<SectionListData<TItem, SectionWithIndexOffset<TItem>>> | null,
  ...
  const renderSectionHeader = ({
    ...
    section: SectionListDataType<TItem>;
  }) => {
      ...
      {!headerMessage && canSelectMultiple && shouldShowSelectAll && (
        <View
          ...
          <View style={[styles.flexRow, styles.alignItemsCenter]}>
            <Checkbox
              accessibilityLabel={translate('session.people.selectAll')}
            ...
            {/* {!customListHeader && (
              <PressableWithFeedback
                ...
                <Text style={[styles.textStrong, styles.ph3]}>
  ...
  const renderItem = ({
    ...
    index,
    section,
  }: SectionListRenderItemInfo<TItem, SectionWithIndexOffset<TItem>>) => {
    const normalizedIndex = index + (section?.indexOffset ?? 0);
    const isDisabled = !!section.isDisabled || item.isDisabled;
    const isItemFocused =
      (!isDisabled || item.isSelected) &&
      (focusedIndex === normalizedIndex ||
        itemsToHighlight?.has(item.keyForList ?? ''));
    // We only create tooltips for the first 10 users or so
  ...
  const scrollToFocusedIndexOnFirstRender = useCallback(
    (nativeEvent: LayoutChangeEvent) => {
      if (shouldUseDynamicMaxToRenderPerBatch) {
        ...
        setMaxToRenderPerBatch(
          (Math.ceil(listHeight / itemHeight) || 0) +
            CONST.MAX_TO_RENDER_PER_BATCH.DEFAULT,
        );
      }
  ...
  const prevSelectedOptionsLength = usePrevious(
  ...
  useEffect(() => {
    ...
    const newSelectedIndex =
      textInputValue === '' ||
      (flattenedSections.selectedOptions.length !== prevSelectedOptionsLength &&
        prevAllOptionsLength === flattenedSections.allOptions.length)
        ? -1
        : 0;
  ...
  const scrollAndHighlightItem = useCallback(
    (items: string[], timeout: number) => {
      ...
      const index = flattenedSections.allOptions.findIndex(option =>
        newItemsToHighlight.has(option.keyForList ?? ''),
      );
    ...
    <SafeAreaConsumer>
      {({safeAreaPaddingBottomStyle}) => (
        <View
          style={[
            styles.flex1,
            (!isKeyboardShown || !!footerContent || showConfirmButton) &&
              safeAreaPaddingBottomStyle,
            containerStyle,
          ]}>
          {shouldShowTextInput && (
          ...
          {(!isLoadingNewOptions ||
            headerMessage !== translate('common.noResultsFound') ||
            (flattenedSections.allOptions.length === 0 &&
