...
function CustomStatusBarAndBackground({
  isNested = false,
}: CustomStatusBarAndBackgroundProps) {
  const {isRootStatusBarEnabled, setRootStatusBarEnabled} = useContext(
    CustomStatusBarAndBackgroundContext,
  );
  ...
  const updateStatusBarStyle = useCallback(
    (listenerID?: number) => {
      ...
      // appBG color.
      let currentRoute:
        | ReturnType<typeof navigationRef.getCurrentRoute>
        | undefined;
      ...
      if (
        currentRoute &&
        'name' in currentRoute &&
        currentRoute.name in theme.PAGE_THEMES
      ) {
      ...
      if (
        !didForceUpdateStatusBarRef.current &&
      ...
      if (
        currentScreenBackgroundColor !== theme.appBG ||
