...
function FormProvider(
  ...
  const hasServerError = useMemo(
    () => !!formState && !isEmptyObject(formState?.errors),
    [formState],
  );
  ...
  const onValidate = useCallback(
    (values: FormOnyxValues, shouldClearServerError = true) => {
      ...
      const validateErrors: GenericFormInputErrors =
        validate?.(trimmedStringValues) ?? {};
      ...
      Object.entries(trimmedStringValues).forEach(([inputID, inputValue]) => {
        ...
        const foundHtmlTagIndex = inputValue.search(
          CONST.VALIDATE_FOR_HTML_TAG_REGEX,
        );
        const leadingSpaceIndex = inputValue.search(
          CONST.VALIDATE_FOR_LEADINGSPACES_HTML_TAG_REGEX,
        );
        ...
        const matchedHtmlTags = inputValue.match(
          CONST.VALIDATE_FOR_HTML_TAG_REGEX,
        );
        let isMatch = CONST.WHITELISTED_TAGS.some(regex =>
      ...
      const touchedInputErrors = Object.fromEntries(
        Object.entries(validateErrors).filter(
          ([inputID]) => touchedInputs.current[inputID],
        ),
      );
    ...
    (optionalValue: FormOnyxValues) => {
      Object.keys(inputValues).forEach(inputID => {
        setInputValues(prevState => {
          ...
          copyPrevState[inputID] =
            optionalValue[inputID as keyof FormOnyxValues] || '';
  ...
  const registerInput = useCallback<RegisterInput>(
    (inputID, shouldSubmitForm, inputProps) => {
      const newRef: MutableRefObject<InputComponentBaseProps> =
      ...
      if (inputProps.value !== undefined) {
        ...
        inputProps.shouldSaveDraft &&
        draftValues?.[inputID] !== undefined &&
        inputValues[inputID] === undefined
      ) {
        inputValues[inputID] = draftValues[inputID];
      } else if (
        inputProps.shouldUseDefaultValue &&
        ...
        inputValues[inputID] =
          ...
          getInitialValueByType(inputProps.valueType);
      }
        ...
        onBlur: event => {
          ...
          if (Visibility.isVisible() && Visibility.hasFocus()) {
            ...
            const relatedTargetId =
              relatedTarget &&
              'id' in relatedTarget &&
              typeof relatedTarget.id === 'string' &&
              relatedTarget.id;
