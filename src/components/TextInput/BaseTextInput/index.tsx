...
function BaseTextInput(
  ...
  const labelScale = useRef(
    new Animated.Value(
      initialActiveLabel
        ? styleConst.ACTIVE_LABEL_SCALE
        : styleConst.INACTIVE_LABEL_SCALE,
    ),
  ).current;
  const labelTranslateY = useRef(
    new Animated.Value(
      initialActiveLabel
        ...
        : styleConst.INACTIVE_LABEL_TRANSLATE_Y,
    ),
  ).current;
  ...
  const onLayout = useCallback(
    (event: LayoutChangeEvent) => {
      ...
      setWidth((prevWidth: number | null) =>
        autoGrowHeight ? layout.width : prevWidth,
      );
      setHeight((prevHeight: number) =>
        !multiline ? layout.height : prevHeight,
      );
    },
  ...
  const togglePasswordVisibility = useCallback(() => {
    setPasswordHidden(
      (prevPasswordHidden: boolean | undefined) => !prevPasswordHidden,
    );
  }, []);
  ...
  const newTextInputContainerStyles: StyleProp<ViewStyle> = StyleSheet.flatten([
    ...
    autoGrowHeight && {
      scrollPaddingTop:
        typeof maxAutoGrowHeight === 'number'
          ? 2 * maxAutoGrowHeight
          : undefined,
    },
  ]);
  ...
  const inputPaddingLeft =
    ...
    StyleUtils.getPaddingLeft(
      StyleUtils.getCharacterPadding(prefixCharacter) + styles.pl1.paddingLeft,
    );
  const inputPaddingRight =
    !!suffixCharacter &&
    StyleUtils.getPaddingRight(
      StyleUtils.getCharacterPadding(suffixCharacter) + styles.pr1.paddingRight,
    );
  ...
  return (
    <>
      <View
          ...
          <View
            ...
            <View
              ...
              {iconLeft && (
                <View
                  style={[
                    styles.textInputLeftIconContainer,
                    !isReadOnly
              ...
              {!!prefixCharacter && (
                <View
                  style={[styles.textInputPrefixWrapper, prefixContainerStyle]}>
                  <Text
                    tabIndex={-1}
                    style={[
              ...
              {!!suffixCharacter && (
                <View
                  style={[styles.textInputSuffixWrapper, suffixContainerStyle]}>
                  <Text
                    tabIndex={-1}
                    style={[
              ...
              {!!inputProps.secureTextEntry && (
                <Checkbox
                  ...
                  <Icon
                    src={
                      passwordHidden ? KirokuIcons.Eye : KirokuIcons.EyeDisabled
                    }
                    fill={theme.icon}
                  />
                </Checkbox>
        ...
        {!!inputHelpText && (
          <FormHelpMessage isError={!!errorText} message={inputHelpText} />
        )}
      </View>
      {contentWidth && (
        <View
          ...
          <Text
            style={[
              inputStyle,
              autoGrowHeight &&
                styles.autoGrowHeightHiddenInput(
                  width ?? 0,
                  typeof maxAutoGrowHeight === 'number'
                    ? maxAutoGrowHeight
                    : undefined,
