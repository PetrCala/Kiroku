...
function datetimeToCalendarTime(
  ...
  const date = getLocalDateFromDatetime(
    locale,
    datetime,
    currentSelectedTimezone,
  );
  ...
  if (date >= startOfCurrentWeek && date <= endOfCurrentWeek) {
    return `${format(date, CONST.DATE.MONTH_DAY_ABBR_FORMAT)} ${at} ${format(
      date,
      CONST.DATE.LOCAL_TIME_FORMAT,
    )}${tz}`;
  }
  return `${format(date, CONST.DATE.MONTH_DAY_YEAR_ABBR_FORMAT)} ${at} ${format(
    date,
    CONST.DATE.LOCAL_TIME_FORMAT,
  )}${tz}`;
}
...
function getMonthNames(preferredLocale: Locale): string[] {
  ...
  return monthsArray.map(monthDate =>
    format(monthDate, CONST.DATE.MONTH_FORMAT),
  );
}
 ...
 */
function get12HourTimeObjectFromDate(dateTime: string): {
  hour: string;
  minute: string;
  period: string;
} {
...
function areDatesIdentical(
  dateTimeStringFirst: string,
  dateTimeStringSecond: string,
): boolean {
 ...
 * returns {Boolean}
 */
const isTimeAtLeastOneMinuteInFuture = ({
  timeString,
  dateTimeString,
}: {
  timeString?: string;
...
function formatToSupportedTimezone(timezoneInput: Timezone): Timezone {
  ...
  return {
    selected:
      timezoneBackwardMap[timezoneInput.selected] ?? timezoneInput.selected,
