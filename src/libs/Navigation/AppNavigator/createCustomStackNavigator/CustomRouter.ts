...
function insertRootRoute(
  ...
  routeToInsert: NavigationPartialRoute,
) {
  const nonModalRoutes = state.routes.filter(
    route =>
      route.name !== NAVIGATORS.RIGHT_MODAL_NAVIGATOR &&
      route.name !== NAVIGATORS.LEFT_MODAL_NAVIGATOR,
  );
...
function compareAndAdaptState(state: StackNavigationState<RootStackParamList>) {
  ...
  if (topmostBottomTabRoute && !isNarrowLayout) {
    ...
    const {adaptedState: templateState} = getAdaptedStateFromPath(
      pathFromCurrentState,
    ...
    const templateFullScreenRoute = templateState.routes.find(
      route => route.name === NAVIGATORS.FULL_SCREEN_NAVIGATOR,
    );
    ...
    const templateCentralPaneRouteExtracted = getTopmostCentralPaneRoute(
      templateState as State<RootStackParamList>,
    );
    ...
    if (
      topmostCentralPaneRouteExtracted &&
      templateCentralPaneRouteExtracted &&
      topmostCentralPaneRouteExtracted.name !==
        templateCentralPaneRouteExtracted.name
    ) {
      ...
      // TODO validate this is correctly commented
