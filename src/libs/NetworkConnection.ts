...
function subscribeToNetInfo(): () => void {
  ...
  if (!CONFIG.IS_USING_LOCAL_WEB) {
    ...
    NetInfo.configure({
      ...
      reachabilityUrl: `${CONFIG.KIROKU.DEFAULT_API_ROOT}api/Ping?userID=${userID || 'unknown'}`,
        ...
        return response
          ...
          .then((json: ResponseJSON) => {
            if (json.jsonCode !== 200 && isServerUp) {
              Log.info(
                '[NetworkConnection] Received non-200 response from reachability test. Setting isServerUp status to false.',
              );
              isServerUp = false;
              wasServerDown = true;
            } else if (json.jsonCode === 200 && !isServerUp) {
              Log.info(
                '[NetworkConnection] Received 200 response from reachability test. Setting isServerUp status to true.',
              );
              isServerUp = true;
            }
  ...
  const unsubscribeNetInfo = NetInfo.addEventListener(state => {
    ...
    setOfflineStatus(
      state.isInternetReachable === false || !isServerUp,
      'NetInfo received a state change event',
    );
    Log.info(
      `[NetworkStatus] NetInfo.addEventListener event coming, setting "offlineStatus" to ${!!state.isInternetReachable} with network state: ${JSON.stringify(state)}`,
    );
    let networkStatus;
    if (!isBoolean(state.isInternetReachable)) {
      networkStatus = CONST.NETWORK.NETWORK_STATUS.UNKNOWN;
    } else {
      networkStatus = state.isInternetReachable
  ...
  const recheckIntervalID = setInterval(() => {
    ...
    Log.info(
      `[NetworkStatus] Rechecking the network connection with "isOffline" set to "true" to double-check internet reachability.`,
    );
...
function clearReconnectionCallbacks() {
  Object.keys(reconnectionCallbacks).forEach(
