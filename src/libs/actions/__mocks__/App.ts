...
const ApplyUpdatesImplementation = jest.requireActual<
  typeof ApplyUpdatesImport
>('@libs/actions/OnyxUpdateManager/utils/applyUpdates');
const getMissingOnyxUpdates = jest.fn((_fromID: number, toID: number) => {
  ...
  return ApplyUpdatesImplementation.applyUpdates(
    mockValuesProxy.missingOnyxUpdatesToBeApplied,
  );
});
