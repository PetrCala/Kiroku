...
export default {
  ...
  liveSessionScreen: {
    ...
    timeline: 'Časová osa',
    timelineEmpty: 'Zaznamenané drinky se objeví tady, od nejstaršího.',
    entryMultiplier: ({count}: EntryCountParams) => `x${count}`,
    entryLabel: ({drink, time}: DrinkAtTimeParams) => `${drink} v ${time}`,
    entryOptions: ({drink, time}: DrinkAtTimeParams) =>
      `Možnosti pro ${drink} v ${time}`,
    editDrink: 'Upravit drink',
    deleteDrink: 'Smazat drink',
    drinkOptions: ({drink}: DrinkNameParams) =>
      `Další způsoby, jak přidat ${drink}`,
    addDrinkTitle: ({drink}: DrinkNameParams) => `Přidat ${drink}`,
    addDrink: 'Přidat',
    servingLabel: 'Porce',
    serving: ({ml, abv}: DrinkServingParams) => `${ml} ml, ${abv} %`,
    whenLabel: 'Kdy',
    justNow: 'Právě teď',
    minutesAgo: ({minutes}: MinutesAgoParams) => `před ${minutes} min`,
    countLabel: 'Kolik',
    timeLabel: 'Čas',
    addedDrink: ({drink}: DrinkNameParams) => `Přidáno: ${drink}.`,
    undo: 'Vrátit',
    sessionTimes: 'Čas začátku a konce',
    source: {
      watch: 'Hodinky',
      liveActivity: 'Uzamčená obrazovka',
      web: 'Web',
    },
  },
  sessionTimesScreen: {
    title: 'Časy relace',
    prompt:
      'Nastavte, kdy tato relace začala a skončila. Datum zůstane stejné, to změníte na obrazovce s datem.',
    start: 'Začátek',
    end: 'Konec',
    hours: 'Hodiny',
    minutes: 'Minuty',
    error: {
      ...
      endBeforeStart: 'Relace nemůže skončit dříve, než začala.',
    },
  },
  sessionDateScreen: {
    title: 'Datum relace',
