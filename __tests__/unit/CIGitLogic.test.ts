...
//     Log.warn(
//       `Found existing directory at ${DUMMY_DIR}, deleting it to simulate a fresh checkout...`,
//     );
...
//   exec(
//     'git fetch --no-tags --prune --progress --no-recurse-submodules --depth=1 origin +refs/heads/master:refs/remotes/origin/master',
//   );
//   exec('git checkout --progress --force -B master refs/remotes/origin/master');
//   Log.success('Checked out repo at $DUMMY_DIR!');
// }

// function bumpVersion(level: SemverLevel) {
//   Log.info('Bumping version...');
//   setupGitAsKirokuAdmin();
...
//   Log.success(`Version bumped to ${nextVersion} on master`);
// }

// function updateStagingFromMaster() {
//   Log.info('Recreating staging from master...');
//   exec('git switch master');
//   try {
...
//   Log.success('Recreated staging from master!');
// }
...
//   Log.info(`Merging PR #${num} to master`);
//   exec('git switch master');
//   exec(
//     `git merge ${branchName} --no-ff -m "Merge pull request #${num} from /${branchName}"`,
//   );
//   exec('git push origin master');
//   exec(`git branch -d ${branchName}`);
//   Log.success(`Merged PR #${num} to master`);
// }

// function cherryPickPR(
//   num: number,
//   resolveVersionBumpConflicts: () => void = () => {},
//   resolveMergeCommitConflicts: () => void = () => {},
// ) {
//   Log.info(`Cherry-picking PR ${num} to staging...`);
//   mergePR(num);
//   const prMergeCommit = execSync('git rev-parse HEAD', {
//     encoding: 'utf-8',
//   }).trim();
//   bumpVersion(VersionUpdater.SEMANTIC_VERSION_LEVELS.BUILD);
//   const versionBumpCommit = execSync('git rev-parse HEAD', {
//     encoding: 'utf-8',
//   }).trim();
//   checkoutRepo();
//   setupGitAsKirokuAdmin();
...
//   exec(
//     `git fetch origin master staging --no-tags --shallow-exclude="${previousPatchVersion}"`,
//   );
...
//   try {
//     exec(`git cherry-pick -x --masterline 1 ${versionBumpCommit}`);
//   } catch (e) {
...
//   try {
//     exec(
//       `git cherry-pick -x --masterline 1 --strategy=recursive -Xtheirs ${prMergeCommit}`,
//     );
//   } catch (e) {
...
//   exec('git switch staging');
//   exec(
//     `git merge cherry-pick-staging --no-ff -m "Merge pull request #${num + 1} from /cherry-pick-staging"`,
//   );
...
// async function assertPRsMergedBetween(
//   from: string,
//   to: string,
//   expected: number[],
// ) {
...
//   Log.success(
//     `Verified PRs merged between ${from} and ${to} are [${expected.join(',')}]`,
//   );
// }
...
//     Log.info('Reverting the append + prepend on master in PR #9');
//     setupGitAsHuman();
//     exec('git switch master');
//     exec('git switch -c pr-9');
//     console.log(
//       'RORY_DEBUG BEFORE:',
//       fs.readFileSync('myFile.txt', {encoding: 'utf8'}),
//     );
...
//     Log.info(
//       'Verifying that the revert is present on staging, but the unrelated change is not',
//     );
//     expect(fs.readFileSync('myFile.txt', {encoding: 'utf8'})).toBe(
//       initialFileContent,
//     );
//     expect(fs.existsSync('anotherFile.txt')).toBe(false);

//     Log.info(
//       'Repeating previously reverted append + prepend on master in PR #10',
//     );
//     setupGitAsHuman();
//     exec('git switch master');
...
//     for (let i = 0; i < 3; i++) {
//       exec(
//         `npm --no-git-tag-version version ${VersionUpdater.incrementVersion(getVersion(), VersionUpdater.SEMANTIC_VERSION_LEVELS.MAJOR)}`,
//       );
//     }
//     exec('git add package.json');
...
//     const packageJSONBefore = fs.readFileSync('package.json', {
//       encoding: 'utf-8',
//     });
//     cherryPickPR(
//       14,
