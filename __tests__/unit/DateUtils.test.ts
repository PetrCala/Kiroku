...
describe('DateUtils', () => {
  ...
  it('should return a date object with the formatted datetime when calling getLocalDateFromDatetime', () => {
    const localDate = DateUtils.getLocalDateFromDatetime(
      LOCALE,
      datetime,
      timezone,
    );
  ...
  it('should fallback to current date when getLocalDateFromDatetime is failing', () => {
    const localDate = DateUtils.getLocalDateFromDatetime(
      LOCALE,
      undefined,
      'InvalidTimezone' as SelectedTimezone,
    );
  ...
  it('should return the date in calendar time when calling datetimeToCalendarTime', () => {
    ...
    expect(DateUtils.datetimeToCalendarTime(LOCALE, today)).toBe(
      'Today at 2:32 PM',
    );

    const tomorrow = addDays(
      setMinutes(setHours(new Date(), 14), 32),
      1,
    ).toString();
    expect(DateUtils.datetimeToCalendarTime(LOCALE, tomorrow)).toBe(
      'Tomorrow at 2:32 PM',
    );

    const yesterday = setMinutes(
      setHours(subDays(new Date(), 1), 7),
      43,
    ).toString();
    expect(DateUtils.datetimeToCalendarTime(LOCALE, yesterday)).toBe(
      'Yesterday at 7:43 AM',
    );

    const date = setMinutes(
      setHours(new Date('2022-11-05'), 10),
      17,
    ).toString();
    expect(DateUtils.datetimeToCalendarTime(LOCALE, date)).toBe(
      'Nov 5, 2022 at 10:17 AM',
    );

    const todayLowercaseDate = setMinutes(
  ...
  it('should return the date in calendar time when calling datetimeToRelative', () => {
    ...
    expect(DateUtils.datetimeToRelative(LOCALE, aFewSecondsAgo)).toBe(
      'less than a minute ago',
    );

    const aMinuteAgo = subMinutes(new Date(), 1).toString();
    expect(DateUtils.datetimeToRelative(LOCALE, aMinuteAgo)).toBe(
      '1 minute ago',
    );

    const anHourAgo = subHours(new Date(), 1).toString();
    expect(DateUtils.datetimeToRelative(LOCALE, anHourAgo)).toBe(
      'about 1 hour ago',
    );
  });
  ...
  describe('getDBTime', () => {
    it('should return the date in the format expected by the database', () => {
      ...
      expect(getDBTime).toBe(
        format(new Date(getDBTime), CONST.DATE.FNS_DB_FORMAT_STRING),
      );
    });

    it('should represent the correct date in utc when used with a standard datetime string', () => {
      const timestamp =
        'Mon Nov 21 2022 19:04:14 GMT-0800 (Pacific Standard Time)';
  ...
  describe('formatWithUTCTimeZone', () => {
    ...
    describe('when the date is valid', () => {
      ...
      test.each(scenarios)(
        'returns the date as string with the format "$dateFormat"',
  ...
  describe('getLastBusinessDayOfMonth', () => {
    ...
    test.each(scenarios)(
      'returns a last business day based on the input date',
